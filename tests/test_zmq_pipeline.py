...
def test_pipeline():
    ...
    print(f"Frames sent: {NUM_FRAMES} | received: {received}")

    assert not errors, "ZMQ pipeline failures:\n  " + "\n  ".join(errors)
    assert received == NUM_FRAMES, f"expected {NUM_FRAMES} frames, got {received}"
