...
for try in $(seq 1 "$MAX_TRIES"); do
  stop_all
  log "=== attempt $try/$MAX_TRIES — profile28 all sensors ==="
  rm -f "$LOG"
  PYTHONNOUSERSITE=1 FEXBash -c \
    "/usr/bin/python3 -u src/aria_arm64_bridge/receiver.py --interface usb --profile profile28 --streams $STREAMS" \
    > "$LOG" 2>&1 &
  sleep "$HANDSHAKE_S"
