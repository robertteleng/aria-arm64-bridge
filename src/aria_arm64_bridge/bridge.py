...
class AriaBridge:
    ...
    def _build_receiver_cmd(self) -> str:
        """Shell string for ``FEXBash -c``, with every value quoted.

        ``FEXBash -c`` takes a shell string, not an argv, so anything that
        reaches it unquoted is interpreted by the shell. ``device_ip`` and
        ``profile`` typically come from config or a CLI flag, so they are
        quoted rather than trusted. ``/usr/bin/python3`` is explicit because
        under FEXBash a bare ``python3`` can resolve to the host's ARM64 venv,
        which has no SDK.
        """
        argv = [
            "/usr/bin/python3", "-u", self._receiver_script,
            "--interface", self._interface,
            "--zmq-endpoint", self._zmq_endpoint,
            "--profile", self._profile,
        ]
        if self._device_ip:
            argv += ["--device-ip", self._device_ip]
    ...
    def _find_receiver() -> str:
        ...
        receiver = Path(__file__).parent / "receiver.py"
        if receiver.exists():
            return str(receiver)
